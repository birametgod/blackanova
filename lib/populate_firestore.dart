...
class Category {
  ...
  factory Category.fromFirestore(DocumentSnapshot doc) {
    Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
    List<Services> services = (data['services'] as List<dynamic>? ?? [])
        .map((serviceData) => Services.fromFirestore(serviceData))
        .toList();
    return Category(
      name: doc.id,
      services: services,
...
Future<List<Category>> getDataFromFirestore2() async {
  ...
  try {
    ...
    for (DocumentSnapshot categoryDoc in categoryDocuments) {
      ...
      List<Services> services = serviceDocuments.map((serviceDoc) => Services.fromFirestore(serviceDoc)).toList();
...
Future<void> onInit() async {
  ...
  for (Category category in categories) {
    ...
    for (Services service in category.services) {
      print(service.title);
      print(service.types);
    }
  }
...
Future<void> addUserService(String userId, Services service, String category, double price, int time) async {
  try {
    FirebaseFirestore firestore = FirebaseFirestore.instance;
...
Future<void> addToUserServiceCollection(String userId, double price, int time) async {
  try {
    ...
    for (Services service in servicesToAdd) {
...
Future<List<Services>> getCategoryNService() async {
  List<Services> servicesToAdd = [];
  List<Category> categories = await getDataFromFirestore2();
  for (Category category in categories) {
    print(category.name);
    servicesToAdd.addAll(category.services);
    for ( Services service in servicesToAdd) {
      print(service.title);
    }
  }
...
//8IihfI6zhUb78zSCIZO6 = oulimata
//1gprLLuG0DquNK5zetYZ = Yousra
//Fu53ADecnofLWq3KYUbU = Seynabou
//iij1kJfnv5ODntcCZykY = Maria keita
//tF8ivhR9Ne19dH0OgC9q = Sonia
