...
import '../../global_widgets/container_controller.dart';
...
class BookingController extends GetxController {
  ...
  Future<String?> fetchHairdresserIdFromCache() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString('selected_hairdresser_id');
  }
  ...
  Future<void> restartController() async {
    ...
    Get.delete<ContainerController>();

    // Fetch the hairdresser ID from cache
    final prefs = await SharedPreferences.getInstance();
    final cachedId = prefs.getString('selected_hairdresser_id');
    print(cachedId);

    // Reinitialize the controller with the fetched ID
    if (cachedId != null) {
      Get.put(BookingController(id: cachedId));
      Get.put(ContainerController());
      update();
    } else {
      // Handle the case where the ID is not found in cache
      print("No hairdresser ID found in cache.");
    }
  }
