...
class HomeController extends GetxController {
  ...
  Future<void> loadMapTheme() async {
    try {
      // Load the map style
      String mapStyle = await rootBundle.loadString('assets/mapTheme/constatel.json');
      mapTheme = mapStyle;
      // Notify listeners that the map style has been loaded
      update();
    } catch (e) {
  ...
  Future loadBookingsOfStatus({String? statusId}) async {
    try {
      ...
      if (bookingStatuses.isNotEmpty) {
        //_bookings = await _bookingsRepository.all(statusId ?? currentStatus.value, page: page.value);
      }
